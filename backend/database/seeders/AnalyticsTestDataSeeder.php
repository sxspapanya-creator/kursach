...
class AnalyticsTestDataSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // ========== ИЗМЕНЕНО: 6 МЕСЯЦЕВ ==========
        $totalMonths = 6; // ← ИЗМЕНЕНО: 30 → 6
        $now = Carbon::now();

        // ИЗМЕНЕНО: начальный доход (ближе к текущему)
        $baseIncome = 2800;
        ...
        for ($monthOffset = $totalMonths - 1; $monthOffset >= 0; $monthOffset--) {
            ...
            // ИЗМЕНЕНО: медленный тренд для 6 месяцев
            $trendFactor = 1 + ($totalMonths - 1 - $monthOffset) * 0.003;

            // ========== ДОХОДЫ ==========

            $salary = round($baseIncome * $trendFactor * $seasonalIncomeFactor, 0);
            $finalSalary = max(2200, round($salary * (1 + (rand(-20, 20) / 1000)), 2));
        ...
        $this->command->info("✅ Генерация завершена!");
        $this->command->info("📊 Период: " . Carbon::now()->subMonths($totalMonths - 1)->format('F Y') . " - " . Carbon::now()->format('F Y'));
        $this->command->info("👤 ID пользователя: {$userId}");
        $this->command->info("📁 Категорий: " . Category::where('user_id', $userId)->count());
    }
}