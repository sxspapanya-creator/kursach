...
use Carbon\Carbon;
class TwentyMonthsDataSeeder extends Seeder
{
    public function run(): void
    {
        $this->generateData(20, '20 месяцев');
    }

    protected function generateData(int $totalMonths, string $label): void
    {
        ...
        for ($monthOffset = $totalMonths - 1; $monthOffset >= 0; $monthOffset--) {
            ...
            $finalSalary = max(2100, round($salary * (1 + (rand(-25, 25) / 1000)), 2));
            ...
            $targetMonthlyExpense = round($finalSalary * $baseExpensePercent * $seasonalExpenseFactor * (1 + (rand(-80, 80) / 1000)), 2);
