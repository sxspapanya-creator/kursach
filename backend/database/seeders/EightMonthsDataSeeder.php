...
/**
 * Сидер для генерации тестовых данных за 8 ПОЛНЫХ месяцев + текущий месяц
 *
 * Генерирует:
 * - 8 полных месяцев (например, сентябрь, октябрь, ноябрь, декабрь, январь, февраль, март, апрель)
 * - Транзакции в текущем месяце ТОЛЬКО за прошедшие дни (до сегодня)
 *
 * Всего в истории будет 8 полных месяцев + частичный текущий месяц
 *
 * Запуск: php artisan db:seed --class=EightMonthsDataSeeder
 *
 * ВНИМАНИЕ: Перед запуском очистите данные пользователя самостоятельно!
 */
class EightMonthsDataSeeder extends Seeder
{
    public function run(): void
    {
        // Генерируем 8 полных месяцев (без текущего)
        $completeMonths = 8;
        $label = '8 полных месяцев + текущий';
        ...
        // ========== 1. ГЕНЕРАЦИЯ 8 ПОЛНЫХ МЕСЯЦЕВ (без текущего) ==========
        // Начинаем с месяца, который был 8 месяцев назад
        for ($i = $completeMonths; $i >= 1; $i--) {
            ...
            foreach ($expenseData as $categoryName => $data) {
                ...
                for ($j = 0; $j < $transactionCount; $j++) {
                    ...
                    // Крупные покупки в категории Одежда (15% вероятность)
                    ...
                    // Крупные покупки в категории Техника (через Развлечения)
            ...
            // Добавляем аномальные расходы в некоторые месяцы (покупка техники, мебели)
            if (in_array($monthNumber, [9, 12, 3, 6]) && rand(1, 100) <= 35) {
                $bigPurchaseAmount = rand(500, 2000);
